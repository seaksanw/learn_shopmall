...
class _ListProductPageState extends State<ListProductPage> {
  ...
  void readAllData() {
    ...
    print('listProsuctsPage--------------> readAllData()');
    // collectionReference.snapshots().listen((response) {
    collectionReference.get().then((response) {
      List<DocumentSnapshot> snapshots = response.docs; // response.docs;

      int i = 0;
      for (var snapshot in snapshots) {
        print('listProsuctsPage-------------->In for loop');
        print('listProsuctsPage-------------->' + snapshot.get('name'));
        print('listProsuctsPage-------------->' + snapshot.get('imagePath'));
        tempProducts
        ...
        print(
            'listProsuctsPage-------------->In tempProducts[$i].name : ${tempProducts[i].name}');
        i++;
      }
    });
  }
